    ...
    const onDragEnd = (result) => {
        ...
        if (type === "list") {
            ...
            if (destination.index === 0) {
                newOrder = midString(
                    "",
                    initialData.columns[listOrder[0]].order
                );
                ...
                if (destination.index < source.index) {
                    newOrder = midString(
                        initialData.columns[listOrder[destination.index - 1]]
                            .order,
                        initialData.columns[listOrder[destination.index]].order
                    );
                } else {
        ...
        if (startList === endList) {
            ...
            if (destination.index === 0)
                newOrder = midString(
                    "",
                    initialData.tasks[column.taskIds[0]].order
                );
                ...
                if (destination.index < source.index)
                    newOrder = midString(
                        initialData.tasks[column.taskIds[destination.index - 1]]
                            .order,
                        initialData.tasks[column.taskIds[destination.index]]
                            .order
                    );
                else
                    newOrder = midString(
        ...
        if (endList.taskIds.length === 0) newOrder = "n";
            ...
            if (destination.index === 0) {
                newOrder = midString(
                    "",
                    initialData.tasks[endList.taskIds[0]].order
                );
            } else if (destination.index === endList.taskIds.length)
                newOrder = midString(
                    initialData.tasks[endList.taskIds[destination.index - 1]]
                        .order,
                    ""
                );
            else
        ...
        if (
            ...
            dispatch(
                createNewActivity({ text, boardId: currBoard._id }, token)
            );
        ...
        const postListReq = {
            ...
            order:
                totalLists === 0
                    ...
                    : midString(
                          initialData.columns[
                              initialData.columnOrder[totalLists - 1]
                          ].order,
                          ""
                      ),
        };
